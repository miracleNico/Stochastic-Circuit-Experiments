...
def emit_annealed_adder4(path: Path) -> None:
    ...
    lines = [
        ...
        f"    signal sched_state : std_logic_vector(31 downto 0) := x\"{seed_for('ADDER4_RIPPLE', 0):08X}\";",
    ]
    for i in range(n):
    ...
    lines.extend(
        [
            ...
            "            if rst = '1' then",
            "                phase <= 0;",
            f"                sched_state <= x\"{seed_for('ADDER4_RIPPLE', 0):08X}\";",
            "            elsif enable = '1' then",
    ...
    for i, name in enumerate(ham.nodes):
        ...
        lines.extend(
            [
                ...
                f"            SEED            => x\"{seed_for('ADDER4_RIPPLE', i + 1):08X}\"",
                "        )",
                "        port map (",
                "            clk          => clk,",
                "            rst          => rst,",
...
def main() -> None:
    emit_annealed_adder4(ROOT / "src" / "generated_annealed_adder4.vhd")
    print(f"Wrote {ROOT / 'src' / 'generated_annealed_adder4.vhd'}")
