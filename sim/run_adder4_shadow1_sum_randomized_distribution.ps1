...
try {
    ...
    $oldReverseOrder = $env:REVERSE_ORDER
    if ($ReverseOrder) { $env:REVERSE_ORDER = "true" }

    $oldParallelMode = $env:PARALLEL_MODE
    if ($ParallelMode) { $env:PARALLEL_MODE = "true" }

    & $vsim -c -do run_adder4_shadow1_sum_randomized_distribution.do
    ...
    if ($null -eq $oldReverseOrder) { Remove-Item Env:\REVERSE_ORDER -ErrorAction SilentlyContinue } else { $env:REVERSE_ORDER = $oldReverseOrder }
    if ($null -eq $oldParallelMode) { Remove-Item Env:\PARALLEL_MODE -ErrorAction SilentlyContinue } else { $env:PARALLEL_MODE = $oldParallelMode }
    Pop-Location
}
