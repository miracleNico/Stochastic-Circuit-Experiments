...
if {[file exists work_adder4_shadow1_q34_sumdist]} {
    vdel -lib work_adder4_shadow1_q34_sumdist -all
}
...
vlib work_adder4_shadow1_q34_sumdist
vmap work work_adder4_shadow1_q34_sumdist
...
vcom -2008 ../tb/tb_adder4_shadow1_sum_distribution.vhd
...
vsim \
    ...
    work.tb_adder4_shadow1_sum_distribution

set sum_cases 31
set settle_cycles [expr {$block0_cycles + $block1_cycles + $block2_cycles + $block3_cycles + (3 * $copy_cycles)}]
set run_ns [expr {($sum_cases * (8 + ($trials * ($settle_cycles + 1))) + 200) * 10}]
run ${run_ns} ns
