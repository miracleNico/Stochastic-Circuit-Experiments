...
try {
    ...
    $oldBlock4Cycles = $env:BLOCK4_CYCLES
    if ($Block4Cycles -ge 0) { $env:BLOCK4_CYCLES = [string]$Block4Cycles }

    $oldBlock5Cycles = $env:BLOCK5_CYCLES
    if ($Block5Cycles -ge 0) { $env:BLOCK5_CYCLES = [string]$Block5Cycles }

    $oldBlock6Cycles = $env:BLOCK6_CYCLES
    if ($Block6Cycles -ge 0) { $env:BLOCK6_CYCLES = [string]$Block6Cycles }

    $oldBlock7Cycles = $env:BLOCK7_CYCLES
    if ($Block7Cycles -ge 0) { $env:BLOCK7_CYCLES = [string]$Block7Cycles }
    ...
    & $vsim -c -do run_adder8_shadow1_q34_diagnostics.do
    ...
    if ($null -eq $oldBlock4Cycles) { Remove-Item Env:\BLOCK4_CYCLES -ErrorAction SilentlyContinue } else { $env:BLOCK4_CYCLES = $oldBlock4Cycles }
    if ($null -eq $oldBlock5Cycles) { Remove-Item Env:\BLOCK5_CYCLES -ErrorAction SilentlyContinue } else { $env:BLOCK5_CYCLES = $oldBlock5Cycles }
    if ($null -eq $oldBlock6Cycles) { Remove-Item Env:\BLOCK6_CYCLES -ErrorAction SilentlyContinue } else { $env:BLOCK6_CYCLES = $oldBlock6Cycles }
    if ($null -eq $oldBlock7Cycles) { Remove-Item Env:\BLOCK7_CYCLES -ErrorAction SilentlyContinue } else { $env:BLOCK7_CYCLES = $oldBlock7Cycles }
