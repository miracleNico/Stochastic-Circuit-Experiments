...
if {[file exists work_adder4_shadow1_sum_random]} {
    vdel -lib work_adder4_shadow1_sum_random -all
}
...
set reverse_order false
if {[info exists ::env(REVERSE_ORDER)]} {
    set reverse_order $::env(REVERSE_ORDER)
}
set parallel_mode false
if {[info exists ::env(PARALLEL_MODE)]} {
    set parallel_mode $::env(PARALLEL_MODE)
}

vlib work_adder4_shadow1_sum_random
vmap work work_adder4_shadow1_sum_random

vcom -2008 ../src/inv_sc_pkg.vhd
vcom -2008 ../src/lfsr32.vhd
vcom -2008 ../src/spin_node.vhd
vcom -2008 $generated_shadow_vhdl
vcom -2008 ../tb/tb_adder4_shadow1_sum_randomized_distribution.vhd
...
vsim \
    ...
    -gTRIALS=$trials \
    -gREVERSE_ORDER=$reverse_order \
    -gPARALLEL_MODE=$parallel_mode \
    work.tb_adder4_shadow1_sum_randomized_distribution

set sum_cases 31
set schedule_cycles [expr {$block0_cycles + $block1_cycles + $block2_cycles + $block3_cycles + (3 * $copy_cycles)}]
set solve_cycles $schedule_cycles
if {$parallel_mode eq "true"} {
    set solve_cycles $settle_cycles
}
set trial_cycles [expr {$scramble_cycles + $solve_cycles + 1}]
