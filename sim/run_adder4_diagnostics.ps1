...
try {
    ...
    $oldSettleCycles = $env:ADDER_SETTLE_CYCLES
    if ($SettleCycles -ge 0) {
        $env:ADDER_SETTLE_CYCLES = [string]$SettleCycles
    }

    $oldCountCycles = $env:ADDER_COUNT_CYCLES
    if ($CountCycles -ge 0) {
        $env:ADDER_COUNT_CYCLES = [string]$CountCycles
    }

    & $vsim -c -do run_adder4_diagnostics.do
    ...
    if ($null -eq $oldSettleCycles) {
        Remove-Item Env:\ADDER_SETTLE_CYCLES -ErrorAction SilentlyContinue
    }
    else {
        $env:ADDER_SETTLE_CYCLES = $oldSettleCycles
    }

    if ($null -eq $oldCountCycles) {
        Remove-Item Env:\ADDER_COUNT_CYCLES -ErrorAction SilentlyContinue
    }
    else {
        $env:ADDER_COUNT_CYCLES = $oldCountCycles
    }
